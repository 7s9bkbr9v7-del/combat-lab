...
public final class CombatLabRuntime {
  ...
  public static CombatLabRuntime create(
    ...
    combatEvents.subscribePermanent(
        CombatClickEvent.class,
        event -> cpsTracker.recordClicks(event.clickCount(), event.capturedAtNanos()));
    DebugTelemetry debugTelemetry = new DebugTelemetry();
    combatEvents.subscribePermanent(
        TargetChangedEvent.class,
        event -> debugTelemetry.onTargetChanged(event, options.debugLoggingEnabled(), debug));
    combatEvents.subscribePermanent(
