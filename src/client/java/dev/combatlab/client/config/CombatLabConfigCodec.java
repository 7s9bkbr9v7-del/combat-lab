...
public final class CombatLabConfigCodec {
	...
	private static final Set<String> VALID_ATTACHMENT_SIDES = Set.of("LEFT_OF", "RIGHT_OF", "ABOVE", "BELOW");
	...
	private static Map<String, HudModuleConfig> decodeHudModules(JsonObject root) {
		...
		for (Map.Entry<String, JsonElement> entry : moduleObjects.entrySet()) {
			...
			modules.put(entry.getKey(), decodeHudModule(entry.getKey(), entry.getValue().getAsJsonObject()));
		}
		return modules;
	}
	...
	private static HudModuleConfig decodeHudModule(String id, JsonObject moduleObject) {
		...
		module.normalizedX = optionalNormalizedPosition(moduleObject, "normalizedX", "HUD module '" + id + "'");
		module.normalizedY = optionalNormalizedPosition(moduleObject, "normalizedY", "HUD module '" + id + "'");
		module.scale = optionalScale(moduleObject, id);
		module.layout = optionalAllowedString(moduleObject, "layout", VALID_LAYOUTS, "HUD module '" + id + "'");
		module.attachedTo = optionalString(moduleObject, "attachedTo", "HUD module '" + id + "'");
		module.attachmentSide = optionalAllowedString(
				moduleObject,
				"attachmentSide",
				VALID_ATTACHMENT_SIDES,
				"HUD module '" + id + "'"
		);
	...
	private static boolean optionalBoolean(JsonObject root, String name, boolean fallback, String owner) {
	...
	private static int intValue(JsonObject root, String name, String owner) {
		...
		try {
			...
			throw new IllegalArgumentException(owner + " field '" + name + "' must be an integer", exception);
		}
	}
