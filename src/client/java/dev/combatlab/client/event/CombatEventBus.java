...
public final class CombatEventBus {
	...
	public <T extends CombatEvent> void subscribePermanent(Class<T> eventType, Consumer<? super T> subscriber) {
		addSubscriber(eventType, subscriber);
	}

	private <T extends CombatEvent> RegisteredSubscriber addSubscriber(Class<T> eventType, Consumer<? super T> subscriber) {
	...
	private record RegisteredSubscriber(List<Consumer<CombatEvent>> subscribers, Consumer<CombatEvent> wrapper) {
	}
}
