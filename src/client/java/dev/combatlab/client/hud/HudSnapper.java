...
public final class HudSnapper {
  ...
  public static HudSnapResult snapWithGuides(
    ...
    HudRectangle nearbyRectangle =
        new HudRectangle(
            nearby.position().x(), nearby.position().y(), moving.width(), moving.height());
    HudPosition position =
        snapAligned(nearbyRectangle, others, threshold, !nearby.snappedX(), !nearby.snappedY());
    HudRectangle snapped =
        new HudRectangle(position.x(), position.y(), moving.width(), moving.height());
    return new HudSnapResult(
        position,
        guidesForAlignment(moving, snapped, others, threshold, screenWidth, screenHeight));
  }

  public static HudPosition snapToGrid(
      HudRectangle moving, int gridSize, int screenWidth, int screenHeight) {
    if (gridSize <= 1) {
  ...
  public static HudPosition snapToEdges(
    ...
    HudRectangle horizontal =
        new HudRectangle(xSnap.value(), moving.y(), moving.width(), moving.height());
  ...
  private static AxisSnapResult snapNearby(
    ...
    return new AxisSnapResult(
        new HudPosition(xSnap.value(), ySnap.value()), xSnap.snapped(), ySnap.snapped());
  }
  ...
  private static boolean rangesNear(
      int firstStart, int firstEnd, int secondStart, int secondEnd, int threshold) {
  ...
  private static void addDistantModuleEdgeGuides(
    ...
    for (HudRectangle other : others) {
      if (!rangesNear(snapped.y(), snapped.bottom(), other.y(), other.bottom(), threshold)) {
        addEdgeGuides(
            guides,
            HudSnapGuide.Axis.VERTICAL,
            xGeometry(snapped, other),
            original.x(),
            snapped.x(),
            threshold);
      }
      if (!rangesNear(snapped.x(), snapped.right(), other.x(), other.right(), threshold)) {
        addEdgeGuides(
            guides,
            HudSnapGuide.Axis.HORIZONTAL,
            yGeometry(snapped, other),
            original.y(),
            snapped.y(),
            threshold);
      }
  ...
  private static void addEdgeGuides(
    ...
    for (EdgeSnapTarget target : geometry.edgeTargets()) {
      if (snappedPosition == target.position()
          && Math.abs(originalPosition - target.position()) <= threshold) {
  ...
  private record AxisGeometry(int movingSize, int otherStart, int otherEnd) {
    int[] edgeAlignedPositions() {
      return new int[] {otherStart, otherEnd - movingSize, otherEnd, otherStart - movingSize};
    }
