...
public final class KeystrokesHud extends ResizableBaseHudModule {
	...
	public static HudModuleDescriptor descriptor() {
		return new HudModuleDescriptor(DEFINITION, dependencies -> new KeystrokesHud(dependencies.options(), dependencies.debug()));
	}
	...
	protected void renderModule(GuiGraphicsExtractor graphics, HudRenderContext context) {
		...
		if (context.editorPreview() && !input.anyMovementKeyDown() && !input.anyMouseButtonDown()) {
			input = new InputState(input.cps(), true, true, false, false, true, false, false, true, false);
		}
		...
		renderKey(graphics, context.font(), "LMB", 0, MOUSE_Y, mouseWidth, MOUSE_HEIGHT, input.attack());
		renderKey(graphics, context.font(), "RMB", mouseWidth + GAP, MOUSE_Y, mouseWidth, MOUSE_HEIGHT, input.use());
