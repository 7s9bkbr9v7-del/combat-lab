...
public final class HudContextMenu {
		...
		if (type == MenuType.CANVAS) {
			...
			if (submenuIndex >= 0) {
				...
				if (descriptorIndex < disabledModules.size()) {
					moduleActions.enableAt(disabledModules.get(descriptorIndex).id(), addX, addY, screenWidth, screenHeight);
					close();
					return true;
				}
			}
	...
	private void renderSubmenu(GuiGraphicsExtractor graphics, Font font, int mouseX, int mouseY) {
		...
		if (disabledModules.isEmpty()) {
			renderItem(graphics, font, "No disabled modules", bounds.x(), bounds.y(), bounds.width(), false, DISABLED_TEXT_COLOR);
			return;
		}
	...
	private int submenuItemIndexAt(double mouseX, double mouseY) {
		HudRectangleLike bounds = submenuBounds();
		int visibleItems = visibleSubmenuItems(disabledModules().size());
		int index = itemIndexAt(mouseX, mouseY, bounds.x(), bounds.y(), bounds.width(), visibleItems * ITEM_HEIGHT);
		return index >= visibleItems ? -1 : index;
	}

	private int itemIndexAt(double mouseX, double mouseY, int itemX, int itemY, int width, int height) {
		if (mouseX < itemX || mouseX >= itemX + width || mouseY < itemY || mouseY >= itemY + height) {
			return -1;
		}
		return Math.clamp((int) ((mouseY - itemY) / ITEM_HEIGHT), 0, Math.max(0, height / ITEM_HEIGHT - 1));
	}
