...
public final class HudDragController {
  ...
  public HudDragController(
      HudSelection selection, HudModuleSelection moduleSelection, int snapThreshold) {
  ...
  public boolean drag(
      double mouseX, double mouseY, int screenWidth, int screenHeight, boolean snappingDisabled) {
    if (draggedModule == null) {
      return false;
    }
    ...
    List<HudSelection.ModuleRectangle> others =
        selection.enabledModuleRectanglesExcept(draggedModule, screenWidth, screenHeight);
    if (dragMembers.size() > 1) {
      others = others.stream().filter(candidate -> !isDragMember(candidate.module())).toList();
    }
    HudRectangle moving = new HudRectangle(x, y, size.width(), size.height());
    HudSnapResult snapResult =
        snappingDisabled
    ...
    if (dragMembers.size() > 1) {
      dragSelectedModules(
          finalX - draggedStartX, finalY - draggedStartY, screenWidth, screenHeight);
      snapGuides = snappingDisabled ? List.of() : snapResult.guides();
      return true;
    }
    if (snappingDisabled) {
      draggedModule.updatePosition(finalX, finalY, screenWidth, screenHeight);
  ...
  private List<DragMember> dragMembers(int screenWidth, int screenHeight) {
    if (!moduleSelection.hasMultipleSelected() || !moduleSelection.selected(draggedModule)) {
      return List.of(
          new DragMember(
              draggedModule, selection.rectangle(draggedModule, screenWidth, screenHeight)));
    }
    return moduleSelection.selectedModules(selection.modules()).stream()
        .map(
            module ->
                new DragMember(module, selection.rectangle(module, screenWidth, screenHeight)))
        .toList();
  }
  ...
  private void dragSelectedModules(
      int requestedDeltaX, int requestedDeltaY, int screenWidth, int screenHeight) {
  ...
  private HudSnapResult snap(
    ...
    HudRectangle gridAligned =
        new HudRectangle(grid.x(), grid.y(), moving.width(), moving.height());
