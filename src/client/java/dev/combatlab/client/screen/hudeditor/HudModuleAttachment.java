...
final class HudModuleAttachment {
		...
		for (HudSelection.ModuleRectangle candidate : others) {
			...
			if (verticalRangesNear(moving, target, threshold)) {
				if (moving.right() == target.x()) {
					movingModule.attachTo(candidate.module(), HudAttachmentSide.LEFT_OF, moving.y() - target.y());
					return true;
				}
				if (moving.x() == target.right()) {
					movingModule.attachTo(candidate.module(), HudAttachmentSide.RIGHT_OF, moving.y() - target.y());
					return true;
				}
			}
			if (horizontalRangesNear(moving, target, threshold)) {
				if (moving.bottom() == target.y()) {
					movingModule.attachTo(candidate.module(), HudAttachmentSide.ABOVE, moving.x() - target.x());
					return true;
				}
				if (moving.y() == target.bottom()) {
					movingModule.attachTo(candidate.module(), HudAttachmentSide.BELOW, moving.x() - target.x());
					return true;
				}
			}
		}
	...
	private static boolean verticalRangesNear(HudRectangle first, HudRectangle second, int threshold) {
		return first.bottom() + threshold >= second.y() && second.bottom() + threshold >= first.y();
	}

	private static boolean horizontalRangesNear(HudRectangle first, HudRectangle second, int threshold) {
