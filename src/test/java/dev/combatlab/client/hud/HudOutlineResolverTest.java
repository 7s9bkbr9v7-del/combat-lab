...
class HudOutlineResolverTest {
	...
	void removesOnlyTheTouchingPortionOfASide() {
		HudOutlineSegments segments = HudOutlineResolver.visibleSegments(
				new HudRectangle(100, 100, 80, 20),
				List.of(new HudRectangle(150, 120, 60, 20))
		);

		assertEquals(
				List.of(new HudOutlineSegment(99, 150)),
				segments.bottom()
		);
	...
	void subtractsSeveralTouchingIntervalsFromOneSide() {
		HudOutlineSegments segments = HudOutlineResolver.visibleSegments(
				...
				List.of(
						new HudRectangle(110, 120, 20, 20),
						new HudRectangle(150, 120, 30, 20)
				)
		);
	...
	void hidesTheExactCornerPixelsWhenCornersTouch() {
		HudOutlineSegments segments = HudOutlineResolver.visibleSegments(
				new HudRectangle(100, 100, 80, 20),
				List.of(new HudRectangle(180, 120, 60, 20))
		);
	...
	void removesPerpendicularOutlineExtensionsInsideAnOverlappingModule() {
		HudOutlineSegments segments = HudOutlineResolver.visibleSegments(
				new HudRectangle(100, 100, 80, 20),
				List.of(new HudRectangle(50, 60, 60, 41))
		);
	...
	void alignedButSeparatedModulesKeepCompleteOutlines() {
		HudOutlineSegments segments = HudOutlineResolver.visibleSegments(
				new HudRectangle(100, 100, 80, 20),
				List.of(new HudRectangle(100, 300, 80, 20))
		);
